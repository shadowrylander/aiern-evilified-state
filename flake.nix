{
    ...
    description = "Define an `aiernhide' aiern state inheriting from `emacs' state and setting a minimalist list of Vim key bindings (like navigation, search, ...)";
    inputs = rec {
    ...
    outputs = inputs@{ self, flake-utils, settings, ... }: with builtins; with settings.lib; with flake-utils.lib; settings.mkOutputs {
        inherit inputs;
        type = "emacs-nox";
        pname = "aiern-aiernhide-state";
        callPackage = { emacs, pname }: emacs.pkgs.trivialBuild rec {
            inherit pname;
            src = ./.;
            buildInputs = flatten [ emacs propagatedUserEnvPkgs ];
            propagatedUserEnvPkgs = with emacs.pkgs; flatten [ aiern aiern.propagatedUserEnvPkgs bind-map ];
            meta = {
